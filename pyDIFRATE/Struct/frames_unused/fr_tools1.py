...
def frames2data(mol,frame_funs=None,frame_index=None,mode='full',n=100,nr=10,label=None,dt=None):
    ...
    for q,(v,a,t) in enumerate(zip(vecs,avgs,tensors)):
        ...
        if q>0:
            ...
            data[-1].vars['avg_tensors']={'delta':new[0],'eta':new[1],'euler':new[2:],'rho':rho}          
        else:
        ...
        data[-1].vars['D2inf']={'delta':new[0],'eta':new[1],'euler':new[2:],'rho':rho}
...
#    v0=[[apply_index(v0[k][0],fi[k]),apply_index(v0[k][1],fi[k])] for k in range(nf)]
    ...
    tensors.append({'delta':out[0],'eta':out[1],'euler':out[2:],'rho':Davg}) #Returns in a dict
    ...
    avgs.append({'delta':out[0],'eta':out[1],'euler':out[2:],'rho':Davg})
    ...
    for k in range(nf-1):
        ...
        v1,v2=v0[-1] #This is the bond direction in the current frame
        sc0=vft.getFrame(v1,v2)
        Davg=vft.D2(*sc0).mean(axis=-1) #Average of tensor components of frame 0
        out=vft.Spher2pars(Davg)    #Convert into delta,eta,euler angles
        tensors.append({'delta':out[0],'eta':out[1],'euler':out[2:],'rho':Davg}) #Returns in a dict
        ...
        avgs.append({'delta':out[0],'eta':out[1],'euler':out[2:],'rho':D2inf})
...
def Ct(vZ,dt,index=None,vX=None,vY=None,avgs=None,mode='full'):
    ...
    Re(<D^2_01>+<D^2_0-1>), to be multiplied by Re(A_1)
    -Im(<D^2_01>+<D^2_0-1>), to be multiplied by Im(A_1)
    Re(<D^2_02>+<D^2_0-2>), to be multiplied by Re(A_2)
    Im(<D^2_02>+<D^2_0-2>), to be multiplied by Im(A_2)
...
def Ct_ft(vZ,vX=None,vY=None,D2='D20'):
    ...
    if D2.lower()=='all' or D2.lower()=='red21' or D2.lower()=='imd21':
        XZ=list()
        YZ=list()
        for k in range(3):
            for j in range(3):
